...
class InceptionB(nn.Module):
    ...
    def forward(self, x):
        ...
        s2 = self.stream2(x)
        s3 = self.stream3(x)
        return torch.cat([s1, s2, s3], 1)


class SoftAttn(nn.Module):
    """Soft Attention (Sec. 3.1.I)
    The input to a Harmonious Attention module is a 3-D tensor (from paper)
...
class SpatialAttn(nn.Module):
    ...
    def __init__(self):
        ...
        # a conv layer of 3 X 3 filter with stride 2 (9parameters)
        self.conv1 = ConvBlock(1, 1, 3, s=2, p=1)
        # a scaling conv layer (1 parameter)
        self.conv2 = ConvBlock(1, 1, 1)

    def forward(self, x):
        # global cross-channel averaging pooling layer
...
class ChannelAttn(nn.Module):
    """Channel Attention (Sec. 3.1.I.2)
       channel-wise attention (which features or channels are important)
    """
...
class HACNN(nn.Module):
    ...
    def forward(self, x):
        assert x.size(2) == 160 and x.size(3) == 64, "Input size is not 160x64"
        x = self.conv(x)
...
if __name__ == "__main__":
    ...
    model = HACNN(num_classes=751).to(device)
    input = torch.randn(8, 3, 160, 64).to(device)
    y = model(input)
    print(y[0].shape)
    print(y[1].shape)